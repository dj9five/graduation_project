...
<s:form name="ActionForm" method="post" action="#">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
            <td align="center" height="25"><span class="Tabfont"><s:property
                    value="%{#request.company.name}"/>(客户)共享情况</span></td>
        </tr>
    </table>
        ...
        <fieldset style='padding:5px;clear:left;'>
            ...
            <c:if test="${! empty sysUsers}">
                <c:forEach items="${sysUsers}" var="sysUser">
                    <div><input type='checkbox' class='checkbox' name='user' value='13' id='7' disabled
                                checked>${sysUser.cnname}</div>
                </c:forEach>
            </c:if>
            &nbsp;
        </fieldset>
