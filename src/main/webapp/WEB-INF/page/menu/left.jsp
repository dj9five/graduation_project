                ...
                <c:forEach items="${applicationScope.sysMenus}" var="sysMenu">
                    <c:if test="${sysMenu.id.menuModule==sysMenu.id.menuPrivilege}">
                        <itcast:checkMemu module="${sysMenu.id.menuModule}" privilege="${sysMenu.id.menuPrivilege}">
                            <li><img src="${pageContext.request.contextPath}/ui/images${sysMenu.icon}" width="17"
                                     height="17">
                                ...
                                <c:forEach items="${applicationScope.sysMenus}" var="sysMenuSub">
                                    <c:if test="${sysMenuSub.id.menuModule!=sysMenuSub.id.menuPrivilege
                                        ...
                                        <itcast:checkMemu module="${sysMenuSub.id.menuModule}"
                                                          privilege="${sysMenuSub.id.menuPrivilege}">
                                            <ul>
                                                <li><img
                                                        src="${pageContext.request.contextPath}/ui/images${sysMenuSub.icon}"
                                                        width="17" height="17">
                                                    <a href="${pageContext.request.contextPath}${sysMenuSub.url}"
                                                       target="${sysMenuSub.target}">${sysMenuSub.menuName}</a></li>
                                            </ul>
                                        </itcast:checkMemu>
                                    </c:if>
                                </c:forEach>
